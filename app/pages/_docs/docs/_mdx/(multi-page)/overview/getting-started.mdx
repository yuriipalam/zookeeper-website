...
See [System Requirements](/docs/admin-ops/administrators-guide#sc_systemReq) in the Admin guide.
...
available in the
[Logging](/docs/developer/programmers-guide#Logging)
...
[maintenance](/docs/admin-ops/administrators-guide#sc_maintenance) for
more details.

### Connecting to ZooKeeper

<Steps>
  <Step>Start `zkCli.sh` and connect to your ZooKeeper server.</Step>
...
That's it for now. To explore more, see the [Zookeeper CLI](/docs/admin-ops/zookeeper-cli).
...
Examples in the ZooKeeper Programmer's Guide](/docs/developer/programmers-guide#ch_programStructureWithExample) for
...
<Callout type="info">
  If you want to test multiple servers on a single machine, specify the servername
  as _localhost_ with unique quorum &amp; leader election ports (i.e. 2888:3888, 2889:3889,
  2890:3890 in the example above) for each server.X in that server's config file. Of course
  separate _dataDir_s and distinct _clientPort_s are also necessary (in the above replicated
  example, running on a single _localhost_, you would still have three config files).
</Callout>
...
> For details, see the [Configuration Parameters](/docs/admin-ops/administrators-guide#id_multi_address).
